...
If `_oracreek/` is missing, run `/oracreek-bootstrap` (or the oracreek-bootstrap skill) first, then continue.
...
Follow all sections of `/oracreek-plan-feature` (Required reads, Steps, Stop / ask rules, Outputs, Do not).

**Gate:** User answers open questions and approves recommended status (**Proposed**, **Planned**, or **In Design**).

## Phase 2 — Document

Follow `/oracreek-new-story`. If an architecture fork is needed (schema, public API, identity, security, durable integrations), also follow `/oracreek-new-adr`.
...
**Gate:** User confirms implementation plan and scope. Then move story to `_oracreek/stories/in-build/` and fix links.
